...
class EarlyStopping:  # pylint: disable=too-few-public-methods
    ...
    def step(self, current_score: float) -> None:
        ...
        Args:
            current_score: The validation metric score from the current evaluation.
        """
...
class Trainer:  # pylint: disable=too-many-instance-attributes
    ...
    This class manages the training loop, validation, early stopping, and progress tracking.
    It integrates with the model wrapper, dataset processor, MLflow logger, and console logger.
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.dataset_processor = DatasetProcessor(config.dataset, self.model.tokenizer)
        ...
        self.checkpoint_dir = Path(config.model.checkpoint_directory)
    ...
    def train(self) -> None:
        ...
        self.mlflow_logger.start_run("llm-pretraining")
        ...
        with tqdm(range(self.config.training.num_epochs), desc="Epochs") as epoch_bar:
            for epoch in epoch_bar:
                ...
                self.training_logger.increment_epoch()

        self.mlflow_logger.log_model(self.model.model)
